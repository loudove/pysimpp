...
class Cluster(set):
    ...
    def update_connect(self):
        ''' Update the connections of the nodes and theconnections with the other clusters. '''
    ...
    def __update_nodes_connections(self):
        ...
        for n in self:
            ...
            connections += [c for pbc,
                            c in zip(_periodic, n.connections) if not pbc and c[0] < c[1]]
            # keep multiple connections (different in their periodic index)
            periodic_connections += [c for pbc,
                                     c in zip(_periodic, n.connections) if pbc]
    ...
    def __update_shift_self(self, parent, shift):
        ...
        for k, v in connectedto.items():
            ...
            _tmp = {tuple(np.array(_k)-shift) if any(_k)
                    else _k: _v for _k, _v in v.items()}
            v.clear()
            v.update(_tmp)
        self.shift += shift
    ...
    def whole(connected):
        ...
        while len(_connected) > 0:
            ...
            if c0 is None:
                if _debug:
                    print(
                        "** the provided list of clusters to be joined are not connected!")
                return tuple()
            ...
            for c1 in sorted(c0.connectedto, key=len, reverse=True):
                ...
                _directions = sorted(directions.items(),
...
class Assembly(set):
    ...
    def __init__(self, s, uid=-1):
        ...
        self.formula = Assembly.find_formula(
            s)  # find formula (simple syntactic)
    ...
    def find_formula(assembly):
        ...
        try:
            _kinds = Counter([ x.kind for x in assembly])
            _formula = "_".join(["%s_%s" % (str(_k), str(_kinds[_k]))
    ...
    def write(self, dir):
        ...
        f.write(
            '#   step   n   sqrg   b   c   sqk   diff_previous   diff_cumulative   diff_initial\n')
...
class EvolutionTracker():
    ''' Implements a simple evolution tracker for assemplies. ''' 
    ...
    def __update(self, previous, current):
        ''' Update previous assembly from its current state (as identified
            at the a latest time step). '''
        ...
        # not in both previous and current
        _diff_previous = len(previous ^ current) // _lenprevious
        _diff_cumulative = len(
            current - _ref.cumulative) // _lencurrent  # brand new nodes
        # not in both initial and current
        _diff_initial = len(current ^ _ref.initial) // _leninitial
    ...
    def __init__(self):
        ''' Initialize an EvolutionTracker object. '''
    ...
    def update_current(self, assemblies, step):
        ''' Update the state with the assemplies of the current step. '''
        ...
        if not self.previous is None:
            ...
            _node_assembly = (self.previous_node_assembly,
                              self.current_node_assembly)
            ...
            for _ip, _p in enumerate(_assemblies[0]):
                ...
                while not len(_candidates) == 0:
                    _candidates = self._track(
                        _candidates, _node_assembly[i], _assemblies[j], _node_assembly[j], _used[j], _sides[j])
                    i, j = (i+1) % 2, (j+1) % 2
    ...
    def write(self, dir):
        ''' Write down the changes traced during the evolution fo the system.
            These changes are recorded as reaction took place between the
